...
class Pagination extends CI_Controller {
  ...
  private $names = array('tom', 'Bharani', 'fred', 'max', 'timmy', 'russel', 'peter', 'angie', 'megan', 'thomas', 'mitchell', 'sally', 'wilma', 'bob', 'alice', 'rick', 'james', 'walker', 'johnny', 'tj', 'thomas', 'jackson', 'sigmond', 'maxwell', 'sam', 'freddy', 'don');

	public function names()
	{
		$rows = $this->db->query('SELECT item_name FROM pos_g_items');	
		if($rows!=false){
			foreach($rows->result() as $row){
				$array[] = array($row->item_name);				
			}
		}
		return $array;
	}
  /* The index() function gets called on page load. */
	public function index()
	{
		...
		$config['base_url'] = 'http://localhost/posgearweb/Web/index.php/pagination/index/';
